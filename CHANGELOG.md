...
- Batch export no longer cancels when you press Ctrl+C. Use **Stop Batch Export** or **Esc** to cancel.
- Collection instances duplicated by Array or Geometry Nodes on the instance Empty now export every realized copy. Combined Assets and Levels emit one marker per copy; MultiMesh manifests list every transform. Godot still instances the referenced asset - nothing is baked into a single mesh.
- MultiMesh Analyze in the Nexus tab now detects those Geometry Nodes / Array copies instead of showing "No valid Assets detected" while export still succeeded.

## [2.1.1] - 2026-08-13

### Blender Addon (nexus_b2g)

- Animation Library export restores the original armature pose (REST vs POSE) afterwards.
- Failed exports are reported as cancelled; a glTF exporter exception no longer crashes the export run.
- Shader materials re-export after a layout or hash invalidation instead of being skipped as unchanged.
- Compressed mesh optimization is rejected when the file grew after restoring Nexus metadata.
- gltfpack reordering animation channels no longer fails optimization parity.
- Collections nested under Exclude now warn during export instead of staying silent.
- Materials whose export path sits outside the Godot project are no longer skipped as unchanged.

### Godot Addon (nexus_importer)

- Godot no longer crashes with "Message queue out of memory" while importing, including when a texture is still scanning.
- Import no longer crashes on glTF files with an empty scene list.
- Mixed-case or backslash asset paths no longer skip reimport or duplicate queue work on Linux and macOS.
- A stuck batch-export lock is released if the Blender process is already gone (Windows `tasklist` failure no longer keeps the lock alive).
- MultiMesh collision bodies use the collection `collision_layer` / `collision_mask` instead of always layer 1.
- MultiMesh `instance_count` matches valid transforms; invalid entries are skipped instead of leaving empty slots.
